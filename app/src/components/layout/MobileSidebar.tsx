...
const navigation = [
  ...
  { name: 'divider', href: '', icon: null, adminOnly: true },
  ...
  { name: 'divider2', href: '', icon: null, adminOnly: true },
...
export function MobileSidebar({ isOpen, onClose }: MobileSidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className="fixed inset-y-0 left-0 z-50 w-64 glass-strong shadow-2xl lg:hidden animate-in slide-in-from-left duration-300">
        {/* Header */}
        <div className="flex h-16 items-center justify-between border-b border-slate-200/50 px-4 bg-gradient-to-r from-slate-50/70 to-transparent">
          <img 
          ...
          <button
            onClick={onClose}
            className="rounded-lg p-2 hover:bg-slate-50/60 transition-colors"
          >
        ...
        {appConfig.owner.showInFooter && (appConfig.owner.name || appConfig.owner.company) && (
          <div className="absolute bottom-0 left-0 right-0 border-t border-slate-200/50 p-4">
            <div className="rounded-xl bg-gradient-to-br from-slate-100/80 via-slate-50/60 to-accent-50/40 p-4 shadow-sm border border-white/60">
              {appConfig.owner.name && (
                <p className="text-xs font-semibold text-slate-600">{appConfig.owner.name}</p>
              )}
              {appConfig.owner.company && (
                <p className="text-xs text-slate-500/70">{appConfig.owner.name ? `trading as ${appConfig.owner.company}` : appConfig.owner.company}</p>
              )}
            </div>
          </div>
        )}
